  ...
  return (
    <div className="mx-auto max-w-lg space-y-4">
      ...
      <form onSubmit={(e) => { e.preventDefault(); save('/profile', form) }}
        className="space-y-3 rounded-2xl border border-brandborder bg-card p-5 shadow-sm">
        <h2 className="font-semibold">Account</h2>
        <div>
          <label className="mb-1 block text-sm font-medium text-textmuted" htmlFor="pname">Name</label>
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-textmuted" htmlFor="pemail">Email</label>
          <input id="pemail" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required
            className="h-12 w-full rounded-lg border border-gray-300 px-3 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-textmuted" htmlFor="ploc">Language / Wika</label>
      ...
      {/* Push toggle */}
      <div className="flex items-center justify-between rounded-2xl border border-brandborder bg-card p-5 shadow-sm">
        <div>
          <h2 className="font-semibold">Push Notifications</h2>
          <p className="text-xs text-textmuted">Outage alerts and permit updates</p>
        </div>
        <button role="switch" aria-checked={pushEnabled}
          ...
          className={`relative h-8 w-14 rounded-full transition ${pushEnabled ? 'bg-success' : 'bg-gray-300'}`}>
          <span className={`absolute top-1 h-6 w-6 rounded-full bg-white shadow transition-all ${pushEnabled ? 'left-7' : 'left-1'}`} />
        </button>
      </div>
        ...
        {[['current_password', 'Current password'], ['password', 'New password'], ['password_confirmation', 'Confirm new password']].map(([k, label]) => (
          <div key={k}>
            ...
            <input type="password" value={pw[k]} required={k !== 'password_confirmation'}
              ...
              className={`h-12 w-full rounded-lg border px-3 text-sm ${errors[k] ? 'border-danger' : 'border-gray-300'}`} />
          </div>
        ))}
      ...
      {/* Privacy — data export (GDPR-like compliance) */}
      <div className="rounded-2xl border border-brandborder bg-card p-5 shadow-sm">
        <h2 className="font-semibold">Privacy</h2>
        <a href="/profile/export" className="mt-2 inline-flex h-11 items-center gap-2 rounded-lg border border-gray-300 px-4 text-sm font-semibold hover:bg-gray-50">
          <Download className="h-4 w-4" /> Export my data (JSON)
        </a>
      </div>

      {/* App info */}
      <div className="flex items-start gap-2 rounded-2xl border border-brandborder bg-card p-5 text-sm text-textmuted shadow-sm">
